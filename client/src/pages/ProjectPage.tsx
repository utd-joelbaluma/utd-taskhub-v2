...
import { getProject, updateProject, type Project, type ProjectStatus } from '@/services/project.service'
...
import { listTasks, createTask, type Task, type ApiTaskStatus, type ApiTaskPriority } from '@/services/task.service'
...
const TASK_STATUS_BADGE: Record<ApiTaskStatus, { variant: 'backlog' | 'todo' | 'in-progress' | 'review' | 'done' | 'cancelled'; label: string }> = {
...
const TASK_PRIORITY_BADGE: Record<ApiTaskPriority, { variant: 'low' | 'medium' | 'high' | 'urgent'; label: string }> = {
...
function TeamAvatars({ members }: { members: Project['project_members'] }) {
  ...
  return (
    <div className="flex items-center">
      {visible.map((m, i) => (
        <Avatar key={m.user_id} className={`h-9 w-9 border-2 border-surface ${i > 0 ? '-ml-3' : ''}`}>
...
function EditProjectDialog({
  open, onClose, project, profiles, onSaved,
}: {
  open: boolean
  onClose: () => void
  project: Project
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      sprintEnds:  project.sprint_end_date ? project.sprint_end_date.slice(0, 10) : '',
  ...
  function set<K extends keyof typeof EDIT_EMPTY>(key: K, value: typeof EDIT_EMPTY[K]) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const toAdd    = form.memberIds.filter(id => !initialMemberIds.includes(id))
      const toRemove = initialMemberIds.filter(id => !form.memberIds.includes(id) && id !== project.created_by)
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input type="date" value={form.sprintEnds} onChange={e => set('sprintEnds', e.target.value)} />
            </div>
            <div>
              ...
              {form.tags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {form.tags.map(tag => (
                    <span key={tag} className="inline-flex items-center gap-1 text-[11px] bg-muted-subtle text-muted-foreground px-2 py-0.5 rounded-full font-medium">
                      {tag}
                      <button type="button" onClick={() => removeTag(tag)} className="text-muted hover:text-foreground transition-colors">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {profiles.map((profile, i) => {
                  const isOwner = profile.id === project.created_by
                  const checked = form.memberIds.includes(profile.id)
                  return (
                    <label
                      key={profile.id}
                      className={cn(
                        'flex items-center gap-3 px-3 py-2.5 rounded-lg border transition-colors',
                        isOwner ? 'opacity-60 cursor-not-allowed border-border' :
                          checked ? 'border-primary bg-primary-subtle cursor-pointer' : 'border-border hover:bg-muted-subtle cursor-pointer'
                      )}
                    >
                      <Checkbox
                        checked={checked}
                      ...
                      <span className="text-sm text-foreground truncate">
                        ...
                        {isOwner && <span className="ml-1 text-[10px] text-muted">(owner)</span>}
                      </span>
                    </label>
                  )
                })}
...
function NewTaskDialog({
  ...
  function set<K extends keyof typeof TASK_EMPTY>(key: K, value: typeof TASK_EMPTY[K]) {
    setForm(prev => ({ ...prev, [key]: value }))
  ...
  function handleOpenChange(isOpen: boolean) {
    if (!isOpen) { setForm(TASK_EMPTY); setErrors({}) }
    if (!isOpen) onClose()
  }
      ...
      <DialogContent className="max-w-[520px]">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium text-muted-foreground mb-1.5 block">Status</label>
              <Select value={form.status} onValueChange={v => set('status', v as ApiTaskStatus)}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium text-muted-foreground mb-1.5 block">Assigned To</label>
    ...
    <div className="mx-auto max-w-[1280px] px-6 py-8">
      ...
      <div className="flex items-start justify-between gap-6 mb-6">
        <div className="flex-1">
          ...
          <p className="text-sm text-muted leading-relaxed max-w-2xl">{project.description || 'No description.'}</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Button className="flex items-center gap-2" onClick={() => setNewTaskOpen(true)}>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-[280px_1fr] gap-4">
            ...
            <Card className="p-5">
              ...
              <p className="text-xs text-muted mb-3">{doneCount} / {tasks.length} Tasks</p>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="rounded-lg bg-muted-subtle px-3 py-2">
                  ...
                  <p className="text-lg font-bold text-foreground">{tasks.length - doneCount}</p>
                </div>
              </div>
            </Card>
            ...
            <Card className="p-5">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <h2 className="text-2xl font-bold text-foreground">{project.sprint_name || 'No sprint'}</h2>
                  {project.sprint_end_date && (
                    <p className="text-sm text-muted mt-0.5">Ends {formatDate(project.sprint_end_date)}</p>
                  )}
                </div>
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <p className="text-sm text-foreground">
                  <span className="font-semibold">{members.length}</span> team member{members.length !== 1 ? 's' : ''}
                </p>
              </div>

              <div className="flex gap-2">
                {Array.from({ length: sprintBlocks }).map((_, i) => (
                  <div key={i} className={`h-2 flex-1 rounded-full ${i < filledBlocks ? 'bg-primary' : 'bg-border'}`} />
                ))}
              </div>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-[1fr_320px] gap-4">
              ...
              {project.tags.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tags.map(tag => (
                    <span key={tag} className="text-sm bg-muted-subtle text-muted-foreground px-3 py-1 rounded-full font-medium">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            <Card className="p-5">
              <h2 className="text-base font-semibold text-foreground mb-4">Team</h2>
              {members.length === 0 ? (
            ...
            <tbody>
              {tasksLoading ? (
                ...
                tasks.map((task, i) => {
                  ...
                  const priorityInfo = TASK_PRIORITY_BADGE[task.priority]
                  const assignee = task.assigned_to ?? null
                  return (
                    <tr key={task.id} className="border-b border-border last:border-0 hover:bg-muted-subtle transition-colors">
                      <td className="px-5 py-4">
                        <p className="text-sm font-medium text-foreground">{task.title}</p>
                        {task.description && (
                          <p className="text-[11px] text-muted mt-0.5 truncate max-w-xs">{task.description}</p>
                        )}
                      </td>
                      <td className="px-5 py-4">
      ...
      <EditProjectDialog
        ...
        profiles={profiles}
        onSaved={(updated) => {
          setProject(prev => prev ? { ...updated, project_members: prev.project_members } : updated)
        }}
      />
